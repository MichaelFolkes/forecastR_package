...
return(data.frame(dat.df, dat.ranks))
  ...
  #need to exclude the 'total' column from the sum of average ranks
  summed.col <- grep(pattern = "total", x = colnames(age.avg.df), ignore.case = TRUE)
  rank.sum <- rowSums(age.avg.df[,-summed.col])
